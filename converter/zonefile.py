...
class Material(object):
    def __init__(self, flags, textures, param=0):
        self.flags = flags
        self.filenames = tuple(
            hashlib.sha256(data).hexdigest() + ".dds" for data in textures
        )
...
class Mesh(object):
    ...
    def add(self, mesh):
        ...
        if self.vertbuffer is not mesh.vertbuffer:
            ...
            self.polygons += [
                (a + offset, b + offset, c + offset) for a, b, c in mesh.polygons
            ]
        else:
    ...
    def subset(self, cpoly):
        ...
        return Mesh(
            self.material,
            VertexBuffer(vbuffer[0], len(used)),
            npoly,
...
class Light(object):
    def __init__(self, position, radius, attenuation, color, flags):
        self.position = position
        self.radius = radius
...
class Zone(object):
    ...
    def addLight(self, position, radius, attenuation, color, flags):
        light = Light(position, radius, attenuation, color, flags)
    ...
    def output(self, zip):
        ...
        def rotate(data, xs=1, ys=1, zs=1):
            # Simple passthrough for now - would need pyrr for proper rotation
            return data
            # out = []
        ...
        def ostring(x):
            ...
            if sl == 0:
                zout.write(bytes([0]))
            while sl:
                zout.write(bytes([(sl & 0x7F) | (0x80 if sl > 127 else 0x00)]))
                sl >>= 7
            zout.write(x.encode("utf-8"))
        ...
        with tempfile.TemporaryFile() as zout:
            ...
            ouint(len(materials))
            for (flags, param, filenames), id in sorted(
                materials.items(), key=lambda x: x[1]
            ):
                ouint(flags)
                ouint(param)
                ouint(len(filenames))
            ...
            for i, obj in enumerate(self.objects):
                ...
                for mesh in obj.meshes:
					...
					odata = []
					for i in range(len(mesh.vertbuffer)):
						odata += rotate(data[i * 9 + 0 + 0:i * 9 + 0 + 3])
						odata += rotate(data[i * 9 + 3 + 0:i * 9 + 3 + 3])
						odata += data[i * 9 + 6 + 0:i * 9 + 6 + 3]
					"""
                    odata = mesh.vertbuffer.data
                    ofloat(*odata)
