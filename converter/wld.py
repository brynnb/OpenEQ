...
class Wld(object):
            ...
            for meshfrag in self.byType[0x36]:
                ...
                for count, index in meshfrag["polytex"]:
                    ...
                    try:
                        ...
                        print(
                            f"ERROR: Texture access failed for object {meshfrag['_name']}, index={index}"
                        )
                        print(f"ERROR: Available textures: {meshfrag['textures']}")
                        print(
                            f"ERROR: Resolved textures: {meshfrag['textures'].resolve()}"
                        )
                    ...
                    if isinstance(mtex, dict):
                        ...
                        if isinstance(resolved_mtex, dict):
                            ...
                            print(
                                f"ERROR: FragRef resolved to unexpected type: {type(resolved_mtex)}, value: {resolved_mtex}"
                            )
                            off += count
                            continue
                    else:
                        print(
                            f"ERROR: Unexpected mtex type: {type(mtex)}, value: {mtex}"
                        )
                        off += count
                        continue
    ...
    def frag_texlist(self):
        # The texture list format is: [0][texture_ref_id]
        # The first uint is always 0, followed by the texture reference fragment ID
        zero_count = self.b.uint()  # This is always 0
        texture_ref_id = self.b.uint()  # This should reference a 0x30 fragment

        if texture_ref_id > 0:
            # The texture_ref_id appears to be off by some amount
            # Let's try to find the correct 0x30 fragment that references the 0x04 fragment
            # For now, assume it references the fragment directly
            result = [self.getFrag(texture_ref_id)]
        else:
            result = []
